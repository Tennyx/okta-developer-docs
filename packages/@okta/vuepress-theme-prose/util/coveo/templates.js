...
const registerTemplates = () => {
  Coveo.TemplateHelpers.registerTemplateHelper("sourceTag", function(source) {
    let markup = "";
    let commonSource = this.result.raw.commonoktasource[0];
    let customCaption = config.facetValueCaptions[commonSource];
  ...
  Coveo.TemplateHelpers.registerTemplateHelper("recommendedTag", function() {
    let rankingModifier = this.result.rankingModifier;
    let markup = "";
